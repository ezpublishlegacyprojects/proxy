<?php
/**
 * File containing the PHProx class.
 *
 * @package proxy
 * @version //autogentag//
...
class PHProxy
{
   ...
   (
    ...
    function set_response()
    {
        ...
        {
            $GLOBALS["eZProxyFollowLocationCounter"] +=1;
            fclose($this->socket);
	if ( $GLOBALS["eZProxyFollowLocationCounter"] > 3)
	{
            die("Looping more as 3 times ");
	}
        ...
        else
        {
            ...
            if (!in_array($this->content_type, array('text/html', 'application/xml+xhtml', 'application/xhtml+xml', 'text/css')) && (!$this->content_length || (int)$this->content_length <= $this->config['max_file_size'] || $this->config['max_file_size'] == -1 ))
            {
                // Impose no time limit since it might be a large file that would take a long while to download.
                @set_time_limit(0);
                $this->send_response_headers( true );
    ...
    function modify_urls()
    {
        ...
        for ($i = 0, $count_i = count($matches[0]); $i < $count_i; $i++)
        {
            ...
            switch ($tag)
            {
                case 'base':
                    if (isset($attrs['href']))
                    {
                        ...
                        $attrs['href'] = $this->proxify_url($attrs['href'], $rebuild);
                    }
                    break;
                case 'body':
                ...
                case 'applet':
                    if (isset($attrs['codebase']))
                    {
                        $rebuild = true;
                        $temp = $this->base;
                        $this->parse_url($this->proxify_url(rtrim($attrs['codebase'], '/') . '/', $rebuild, false), $this->base);
                        unset($attrs['codebase']);
                    }
                    if (isset($attrs['code']) && strpos($attrs['code'], '/') !== false)
                    {
                        $attrs['code'] = $this->proxify_url($attrs['code'], $rebuild);
                    }
                    if (isset($attrs['object']))
                    {
                        $attrs['object'] = $this->proxify_url($attrs['object'], $rebuild);
                    }
                ...
                case 'object':
                    if (isset($attrs['usemap']))
                    {
                        ...
                        $attrs['usemap'] = $this->proxify_url($attrs['usemap'], $rebuild);
                    }
                    if (isset($attrs['codebase']))
                    {
                        $temp = $this->base;
                    ...
                    if (isset($attrs['data']))
                    {
                        $attrs['data'] = $this->proxify_url($attrs['data'], $rebuild);
                    }
                    if (isset($attrs['classid']) && !preg_match('#^clsid:#i', $attrs['classid']))
                    {
                        $attrs['classid'] = $this->proxify_url($attrs['classid'], $rebuild);
                    }
                    if (isset($attrs['archive']))
                    {
                ...
                case 'param':
                    ...
                    {
                        $attrs['value'] = $this->proxify_url($attrs['value'],$rebuild);
                    }
                    break;
                    ...
                    if (isset($attrs['href']))
                    {
                        $urlparsed = parse_url($attrs['href']);
                        $urlhost = $urlparsed['host'];
                        $requesturl = parse_url($this->url);
                        $requesthost = $requesturl['host'];
                    }
                    
                    if (isset($urlhost) and isset($requesthost))
                    {
                        if ($urlparsed['host'] != $requesturl['host'] )
                        {
                            $rebuild = false;
                        }
                        else 
                             ...
                             foreach ($tags[$tag] as $attr)
                                {
                                    if (isset($attrs[$attr]))
                                    {
                                        $attrs[$attr] = $this->proxify_url($attrs[$attr],$rebuild);
                                    }
                                }
                        }
                ...
                case 'form':
                    if (isset($attrs['action']))
                    {
                        ...
                        {
                            $rebuild = true;
                            $extra_html = '<input type="hidden" name="' . $this->config['get_form_name'] . '" value="' . encode_url($this->proxify_url($attrs['action'], $rebuild, false)) . '" />';
                            $attrs['action'] = '';
                            break;
                        }
                    } 
            ...
            if ( isset( $attrs['style'] ) ) # and preg_match( '#url\s*\(\s*#i', $attr['style'] )
            {
                $rebuild = true;
                $attrs['style'] = $this->proxify_css( $attrs['style'] );
            }
            if ($rebuild)
            {
                $new_tag = "<$tag";
    ...
    function follow_location()
    {
        if ( $this->response_code > 299 and $this->response_code < 400 and preg_match("#(location|uri|content-location):([^\r\n]*)#i", $this->response_headers, $matches))
        {
	        ...
	        $rebuild = false;
            $this->url = encode_url($this->proxify_url($url, $rebuild, false));
            return true;
        }
        return false;
    }
